...
for filename in os.listdir('targets'):
	if filename.endswith('png'):
		template = cv2.imread("targets/" + filename)
		...
		for mainImgName in  listOfFiles:
			...
			#(tH, tW) = mainImg.shape[:2]
			result = cv2.matchTemplate(mainImg, template, cv2.TM_CCOEFF)
			(_, maxVal, _, maxLoc) = cv2.minMaxLoc(result)
			if(maxVal > maxNum):
				maxNum = maxVal
				maxFileName = mainImgName
		listOfFiles.remove(maxFileName)
		print filename + " is a subimage of " + maxFileName
		maxNum = 0
"""
